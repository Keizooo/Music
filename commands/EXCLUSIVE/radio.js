...
const { getStations } = require('radio-browser')
  ...
  run: async(client,message,args) => {
    ...
    getStations({
			...
			.then(async data => {
				if (!data[0]) return message.channel.send('<a:ccross:916689400199516210> No radio found with that name');
				...
				try {
					...
					return message.reply('<a:ccross:916689400199516210> You didn\'t choose a song in time.');
				}
				...
				if (index < 0 || index > max - 1) return message.reply(`<a:ccross:916689400199516210> The number you provided was too small or too big (1-${max}).`);
