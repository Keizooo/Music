...
if (!args[0]) {
			...
			const msg = await message.channel.send(`<a:loading1:904369053144068096> Turning on **soft**.`);
	...
	player.setEQ(Array(13).fill(0).map((n, i) => ({
        ...
        const msg = await message.channel.send(`<a:loading1:904369053144068096> Turning off **soft mode**.`);
			const embed = new MessageEmbed()
