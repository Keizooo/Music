...
if (isNaN(args[0])) return message.channel.send('Amount must be a real number.');
		...
		const msg = await message.channel.send(`<a:loading1:904369053144068096> Setting pitch to **${args[0]}**.`);
		const embed = new discord.MessageEmbed()
		.setColor("#00000")
			.setDescription(`<a:ttick:916689402531561492> Pitch set to: **${args[0]}**`);
		await delay(5000);
 	...
 	if (args[0].toLowerCase() == 'reset' || args[0].toLowerCase() == 'off') {
		player.clearEffects()
		const msg = await message.channel.send(`<a:loading1:904369053144068096> Turning off **Pitch**.`);
